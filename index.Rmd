...
数学符号和公式 $\alpha, \beta, \boldsymbol{\alpha}, \bm{\beta}$ 和 $\mathbb{A},\mathsf{A},\mathcal{A},\mathscr{A}$
...
```{r regression, dev = 'tikz', fig.cap = "线性回归模型", fig.ext='tex', out.width="75%", fig.process=to_png, cache=TRUE, fig.align='center', fig.asp=1, fig.width=4, fig.height=4}
x <- rnorm(10)
y <- x + rnorm(5, sd = 0.25)
model <- lm(y ~ x)
...
```{python matplotlib, fig.cap = "matplotlib 示例", dev = ifelse(knitr::is_html_output(), 'svg', ifelse(knitr::is_latex_output(), 'pdf', 'png')), out.width='75%', pdfcrop = FALSE}
